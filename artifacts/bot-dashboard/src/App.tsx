...
function getAvatarUrl(profile: { discord_id?: string; avatar_url?: string | null }) {
...
function ProgressCard({ label, value, goal, helper }: { label: string; value: number; goal: number; helper: string }) {
...
function TranscriptPreview({ transcript }: { transcript: Record<string, string | null | undefined> }) {
  return (
...
function App() {
  const [session, setSession] = useState<DashboardData>(null);
  ...
  const loadDashboard = useCallback(async (activeSession: DashboardData) => {
  ...
  const refreshStaffLeaderboard = useCallback(async (activeSession: DashboardData) => {
    ...
    try {
      ...
      setDashboard((current: DashboardData) => ({
        ...current,
        staff: data.staff || [],
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }: DashboardData) => {
      setSession(data.session || null);
      loadDashboard(data.session || null);
    });

    const { data: listener } = supabase.auth.onAuthStateChange((_event: string, nextSession: DashboardData) => {
  ...
  async function saveModCheck(event: React.FormEvent) {
    ...
    try {
      ...
      setDashboard((current: DashboardData) => ({ ...current, modCheck: data.modCheck, settings: data.settings }));
      setModForm((current: typeof modForm) => ({
        ...current,
      ...
      {showDashboard ? <main className="dashboard-grid">
          ...
          <section className="card admin-panel">
                ...
                <div className="form-grid two-col">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => setModForm((current) => ({ ...current, weekly_ticket_goal: Number(event.target.value) }))}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => setModForm((current) => ({ ...current, message_goal: Number(event.target.value) }))}
                    />
                  </label>
                </div>
              </div>
            ...
            <div className="staff-table-card">
              ...
              <div className="staff-table">
                ...
                {(dashboard.staff || []).map((staffer: DashboardData) => (
        ...
        <section className="card transcripts-panel">
          ...
          <div className="transcript-list">
            {(dashboard.transcripts || []).map((transcript: DashboardData) => (
